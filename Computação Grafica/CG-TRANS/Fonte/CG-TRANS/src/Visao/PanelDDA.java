...
public class PanelDDA extends javax.swing.JPanel {
		...
		try {
			...
			this.setLayout(null);
			{
				...
				}
				{
					...
					jLabelDDA.setText("DDA");
					jLabelDDA.setBounds(121, 14, 28, 20);
	...
	/**
	 * Adiciona linha de coordenadas na JTable
	 */
	public void adicionaLinhaPontos() {  
		...
		removePontos();
		 ...
		 //ADICIONA PONTOS DDA NA JTablePontos
		 for(int i = 0; i < jPanelDesenho.getDDAPontosX().size(); i++){
			 model.addRow( new String [] {""+jPanelDesenho.getDDAPontosX().get(i), ""+jPanelDesenho.getDDAPontosY().get(i)} );  
		 } 
		} 
	...
	private void jButtonDesenharActionPerformed(ActionEvent evt) {
			...
			jPanelDesenho.DDA(Integer.parseInt(jTextFieldX1.getText()), Integer.parseInt(jTextFieldY1.getText()), Integer.parseInt(jTextFieldX2.getText()), Integer.parseInt(jTextFieldY2.getText()));
			adicionaLinhaPontos();
